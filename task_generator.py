...
class MiniImagenetTask(object):
    ...
    def __init__(self, class_folder_ids, character_folders, num_classes, train_num, test_num):
        ...
        class_folders = [character_folders[i] for i in class_folder_ids]
...
def get_mini_imagenet_data_loader(task, num_per_class=1, split='train', shuffle=False):
    normalize = transforms.Normalize(mean=[0.92206, 0.92206, 0.92206], std=[
                                     0.08426, 0.08426, 0.08426])

    dataset = MiniImagenet(task, split=split, transform=transforms.Compose(
